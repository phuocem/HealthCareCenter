...
export default function BookingScreen() {
  ...
  const handleBooking = async () => {
    ...
    setLoading(true);
    try {
      await confirmBookingController(doctorId, date, time, symptoms);
    ...
    >
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
        <View style={{ alignItems: 'center', marginBottom: 32 }}>
          ...
          <Text style={{ fontSize: 28, fontWeight: '800', color: '#1a1a1a' }}>Đặt lịch khám</Text>
          <Text style={{ fontSize: 16, color: '#666', marginTop: 6, textAlign: 'center' }}>
            Chọn ngày giờ và mô tả triệu chứng
          </Text>
        </View>
        ...
        <View style={styles.fieldContainer}>
          <View style={styles.labelContainer}>
            <MaterialCommunityIcons name="file-document-outline" size={22} color="#28A745" />
          ...
          />
          <Text style={{ fontSize: 12, color: '#aaa', marginTop: 6, alignSelf: 'flex-end' }}>
            {symptoms.length}/500
          </Text>
        </View>
        ...
        <Animated.View style={{ transform: [{ scale: scaleValue }] }}>
          ...
          >
            <LinearGradient
              ...
              style={[styles.confirmButton, { flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }]}
            >
              {loading ? (
